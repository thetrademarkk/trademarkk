...
- [x] 2026-06-14 — Journal-app a11y/perf polish (accumulated, pending batch deploy) — accessibility + cohesion hardening of the data-viz surfaces (analytics tabs, insights, tax/reports, dashboard, calendar). Pure tested `src/features/analytics/chart-aria.ts` builds screen-reader summaries; every recharts/SVG chart now carries `role="img"` + a value-bearing `aria-label` (equity curve, group bars, R-histogram, more-stats charts, discipline trend, Monte-Carlo cone, payoff diagram) and a `prefers-reduced-motion`-aware `isAnimationActive` (new `useReducedMotion` hook); NumberFlow KPI stat tiles announce their value via an sr-only string (the rolling digits are aria-hidden); day×time + month heatmap cells gained per-slot aria labels (colour is never the only signal — P&L is always signed); `text-amber-500` → semantic `text-warning`. No behaviour/compute/schema changes. +20 chart-aria unit tests, new `scripts/e2e-journal-a11y.mjs` (18 assertions).
