...
// Track every throwaway user we create so a failure mid-run never leaks
// e2e-authz-* accounts into the platform DB (cleaned in the finally below).
const created = [];
const trackedSignUp = async (tag) => {
  const u = await signUp(tag);
  created.push(u);
  return u;
};
/** Best-effort account-delete for our own throwaway users only. */
const cleanupCreated = async () => {
  for (const u of created) {
    // Hard guard: only ever delete our own e2e-authz-* throwaways.
    if (!/^e2e-authz-[abc]-\d+@example\.com$/.test(u.email)) continue;
    await fetch(`${BASE}/api/account/delete`, {
      method: "POST",
      headers: { Origin: BASE, cookie: u.cookie },
    }).catch(() => undefined);
  }
};
...
let a, b, c;
try {
  a = await trackedSignUp("a");
  b = await trackedSignUp("b");
  ...
  // logged-out → 401 on every DM endpoint
  {
    ...
    check(
      "GET dm/conversations without session → 401",
      inbox.status === 401,
      `got ${inbox.status}`
    );
    ...
    check(
      "POST dm/conversations without session → 401",
      start.status === 401,
      `got ${start.status}`
    );
  ...
  check(
    "A can start a conversation with B → 201",
    startRes.status === 201,
    `got ${startRes.status}`
  );
  const { id: convoId } = await startRes.json();
  {
    ...
    check(
      "restarting the same conversation reuses it",
      dup.id === convoId && dup.created === false
    );
  ...
  // rate-limit smoke: sends must 429 (burst 5/10s or 60/h cap kicks in fast)
  {
    let limited = false;
    ...
    check("DM sends hit 429 (burst 5/10s or 60/h cap)", limited);
  }
  ...
  // account deletion purges content (new purgeUserContent path)
  {
    ...
    check(
      "purged thread is unreachable for B → 404",
      thread.status === 404,
      `got ${thread.status}`
    );
  }
  ...
  // Safety net: delete any of our throwaway users that the happy-path deletes
  // above didn't reach (e.g. an assertion threw mid-run). Idempotent — already
  // deleted users just 404. NEVER touches non-e2e-authz accounts.
