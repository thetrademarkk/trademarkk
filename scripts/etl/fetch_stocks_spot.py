...
# Stock single-stock-F&O universe = UNION of NIFTY-50 + BSE Sensex-30 (the two
# Sensex-only names are ETERNAL = ex-Zomato and INDIGO = InterGlobe Aviation).
# Resolved against the Groww instruments master at runtime; any symbol without a
# CASH listing (LTIM, TATAMOTORS under those exact names today) is logged + skipped.
NIFTY50 = [
...
def main(argv=None) -> int:
    ...
    for sym in syms:
        ...
        for y, m, lo, hi in month_windows(args.start, args.end):
            ...
            empty_marker = path + ".empty.json"
            # IDEMPOTENT: skip months we already have data for OR have already probed
            # and found empty (e.g. ETERNAL/INDIGO before they listed). The empty
            # marker stops every run from re-probing the same dead months — keeps the
            # nightly cron fast and honours "never re-fetch".
            if os.path.exists(path) or os.path.exists(empty_marker):
                continue
            if args.dry_run:
            ...
            if n:
                ...
                # record a permanent-empty marker for a month with no candles, but
                # NOT for the current (in-progress) month — it will fill intraday.
                today = dt.date.today()
                if (y, m) < (today.year, today.month):
                    os.makedirs(os.path.dirname(empty_marker), exist_ok=True)
                    with open(empty_marker, "w", encoding="utf-8") as fh:
                        import json as _json
                        _json.dump({"symbol": sym, "month": f"{y}-{m:02d}",
                                    "reason": "no_candles"}, fh)
