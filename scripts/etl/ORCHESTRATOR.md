# ETL Orchestrator — index + stock options → HuggingFace, disk-safely

`orchestrate.py` is the autonomous, resumable conductor for the
`thetrademarkk/india-index-options-1m` dataset. Its job: fetch index AND
single-stock option-chain 1-minute data at a **±20% strike band on both sides of
ATM**, normalize it, push it to HF, verify the remote, then DELETE the local
source so the disk never fills — and do it on a nightly cron, **without ever
deleting data that isn't proven safe on HF**.
...
- **Indices** (`--track index`): NIFTY, BANKNIFTY, SENSEX. Their 1m spot is
  already on disk; the run **gap-fills only the missing ±20% option strikes**
  across the full available history (~5–6 yr). Existing strikes are never
  re-fetched (the gap detector skips any contract that already has a parquet /
  empty-marker / failure).
- **Stocks** (`--track stocks`, default): the **UNION of NIFTY-50 and BSE
  Sensex-30** constituents = **53 unique names**. Sensex-30 is mostly a NIFTY-50
  subset; the two Sensex-only additions are **ETERNAL** (ex-Zomato) and
  **INDIGO** (InterGlobe Aviation). **LTIM** and **TATAMOTORS** have no current
  single-stock F&O / CASH listing under those exact underlyings in the Groww
  master (corporate actions / renames) and **self-skip** (logged). Resolvable
  stock universe = **51 names**. Stock 1m spot + ±20% option chains, last ~5 yr.

## DISK-SAFE per-(symbol, expiry) slice loop (default LIVE path)

The live run does **NOT** fetch-everything-then-upload (that peaks at the whole
multi-GB dataset on local disk). Instead it processes **ONE (symbol, expiry)
slice end-to-end** and frees its disk before the next:

```
gap-detect (once, read-only) ->
  for each (symbol, expiry) slice:
    gap-fill that slice  ->  normalize -> ONE staging parquet  ->
    upload that ONE file ->  verify remote rowcount == local   ->
    PRUNE that slice (archive source dir + staging file)        ->  next slice
```

So local disk never holds more than ~one expiry's contracts at a time. Every step
is idempotent + resumable: a re-run skips slices already on HF (their archive
source was pruned) and re-attempts anything not yet verified. The legacy
fetch-ALL monolith is still available via `--no-slice-loop` (not recommended —
it peaks at the full dataset).
...
1. **Dry-run is the default.** With no flag, `orchestrate.py` runs read-only:
   planning stages run, fetch runs with `--dry-run`, and stages 4–8 are _printed_
...
- `--strike-pct 0.20` (alias `--target-band-pct`) — strike half-band around ATM
  as a fraction of spot; default **0.20 = ±20% on both sides**, for BOTH stocks
  and index.
- `--max-slices N` — cap (symbol,expiry) slices processed this run (resumable).
- `--only-expiry YYYY-MM-DD` — slice loop processes only that expiry (one-batch proof).
- `--no-slice-loop` — legacy fetch-ALL-then-upload path (peaks at full dataset).
