...
# Stock single-stock-F&O universe = UNION of the NIFTY-50 and the BSE Sensex-30
# constituents (single-stock options are all NSE-listed). The two Sensex-30 names
# NOT in the NIFTY-50 are ETERNAL (ex-Zomato) and INDIGO (InterGlobe Aviation);
# the rest of Sensex-30 is a subset of the NIFTY-50, so the union adds exactly
# those two -> 53 unique names. LTIM and TATAMOTORS have no current single-stock
# F&O / CASH listing under those exact underlying symbols in the Groww master
# (corporate actions / renames), so they self-skip (no spot, no live grid).
FNO_UNIVERSE = [
...
# kept as an alias so any external reference still resolves
NIFTY50 = FNO_UNIVERSE
...
FALLBACK_STEP = {
    ...
    "COALINDIA": 5.0, "DRREDDY": 10.0, "EICHERMOT": 50.0, "ETERNAL": 2.5,
    "GRASIM": 20.0, "HCLTECH": 10.0, "HDFCBANK": 5.0, "HDFCLIFE": 5.0,
    "HEROMOTOCO": 50.0, "HINDALCO": 10.0, "HINDUNILVR": 20.0, "ICICIBANK": 10.0,
    "INDIGO": 50.0, "INDUSINDBK": 10.0,
