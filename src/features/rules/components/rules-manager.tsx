...
export function RulesManager() {
  ...
  return (
    <Card>
      <CardHeader><CardTitle>Your trading rules</CardTitle></CardHeader>
        ...
        <div className="divide-y">
          {rules.map((rule) => (
            <div key={rule.id} className="flex items-center gap-2 py-2">
              <Badge variant="outline" className="capitalize w-20 justify-center">{rule.category}</Badge>
              ...
              <div className="ml-auto flex items-center gap-2">
                <Switch
                  ...
                  onCheckedChange={(active) =>
                    saveRule.mutate({ id: rule.id, text: rule.text, category: rule.category, active })
                  }
                />
                <Button
                  variant="ghost"
                  ...
                  onClick={() => confirm("Delete rule and its history?") && deleteRule.mutate(rule.id)}
                >
