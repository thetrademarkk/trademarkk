...
import { closedOnly, expectancy, netPnl, profitFactor, winRate, maxDrawdown, equityCurve } from "@/lib/stats/stats";
...
function periodRange(kind: PeriodKind, offset: number): { from: string; to: string; label: string } {
  const now = new Date();
  if (kind === "week") {
...
export function ReportView() {
  ...
  const exportCsv = () => {
    ...
    const rows = closed.map((t) =>
      [
        t.opened_at.slice(0, 10), describeInstrument(t).replace(/,/g, " "), t.direction, t.qty,
        t.avg_entry, t.avg_exit ?? "", t.gross_pnl, t.charges, t.net_pnl, t.r_multiple ?? "",
      ].join(",")
    );
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2 print:hidden">
        <Select value={kind} onValueChange={(v) => { setKind(v as PeriodKind); setOffset(0); }}>
        ...
        <Button variant="outline" size="sm" onClick={() => setOffset(offset + 1)}>← Previous</Button>
        <Button variant="outline" size="sm" disabled={offset === 0} onClick={() => setOffset(Math.max(0, offset - 1))}>
          Next →
        </Button>
        <div className="ml-auto flex gap-2">
          <Button variant="outline" size="sm" onClick={exportCsv}><Download className="h-3.5 w-3.5" /> CSV</Button>
          <Button variant="outline" size="sm" onClick={() => window.print()}><Printer className="h-3.5 w-3.5" /> PDF / Print</Button>
        </div>
      </div>

      <Card>
        <CardHeader>
          <CardTitle className="text-base">📊 {range.label} review</CardTitle>
        </CardHeader>
          ...
          {closed.length === 0 ? (
            ...
            <>
              <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
                <div><div className="micro-label">Net P&L</div><PnlText value={netPnl(closed)} className="text-lg font-semibold" /></div>
                <div><div className="micro-label">Trades / Win rate</div><span className="font-money">{closed.length} · {formatPct(winRate(closed), 0)}</span></div>
                <div><div className="micro-label">Profit factor</div><span className="font-money">{Number.isFinite(profitFactor(closed)) ? profitFactor(closed).toFixed(2) : "∞"}</span></div>
                <div><div className="micro-label">Expectancy</div><PnlText value={expectancy(closed)} /></div>
                <div><div className="micro-label">Charges paid</div><span className="font-money">{formatINR(charges)}</span></div>
                <div><div className="micro-label">Max drawdown</div><span className="font-money text-loss">{formatINR(maxDrawdown(equityCurve(closed)))}</span></div>
                {adherence && <div><div className="micro-label">Rule adherence</div><span className="font-money">{formatPct(adherence.overallPct, 0)}</span></div>}
              </div>
